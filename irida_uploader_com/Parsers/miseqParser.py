...
def complete_parse_samples(sample_sheet_file):
    ...
    for sample in sample_list:
        ...
        sample_name_dj=sample.sample_name
        sample_name_dj = sample_name_dj.replace(' ','-')
        sample_name_dj = sample_name_dj.replace('.','-')
        sample_name_dj = sample_name_dj.replace('_','-')
        print sample_name_dj
        
        file_pattern = "{sample_name}_S(\\d+)_L\\d{{3}}_R(\\d+)_\\S+\\.fastq.*$".format(sample_name=re.escape(sample_name_dj),sample_number=sample.sample_number)
        ...
        if not pf_list:
            ...
            file_pattern = "{sample_name}_\\d+_L\\d{{3}}_R(\\d+)_\\S+\\.fastq.*$".format(sample_name=re.escape(sample.get_id()))
            ...
            if not pf_list:
            		file_pattern = "{sample_name}\\_\\d+\\.fastq.*$".format(sample_name=re.escape(sample.get_id()))
            		pf_list = find_file_by_name(directory = data_dir,
                                        name_pattern = file_pattern,
                                        depth = 1)
            		if not pf_list:
    ...
    for sample_number, line in enumerate(csv_reader):
        ...
        for index, key in enumerate(sample_dict.keys()):
           ...
           # print line[index].strip()
            if re.search("sampleName", key):
                sample_name=line[index].strip()
                sample_name = sample_name.replace(' ','-')
                sample_name = sample_name.replace('.','-')
                sample_name = sample_name.replace('_','-')
                sample_dict[key] = sample_name  # assumes values are never empty
            else:
